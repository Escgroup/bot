...
module.exports = class search_command extends Command {
    ...
    async run(message, { text }) {
        const edit_message = await message.channel
            .send("<a:loading:482420749668188170> | Google検索をしています...");
        if (!message.channel.name.match(/bot/)) {
            return edit_message.edit("ログが流れてしまうため、`bot`が含まれていないチャンネルでは使用できません");
        }

        cheerio.set("followMetaRefresh", false);

        cheerio.fetch("http://www.google.com/search", { q: text, hl: "ja", lr: "lang_ja", num: 8 }, (err, $) => {
            if (err) {
            ...
            $("#rso .srg .g").each(function() {
                ...
                const url = $(this).find(".r a").attr("href");
                if (url) {
                    results.push({
                        title: $h3.text(),
                        url: url,
            ...
            return edit_message.edit({
                embed: {
                    ...
                    fields: [
                        { name: `${results[0].title}\n${results[0].url}`, value: results[0].description },
                        { name: `${results[1].title}\n${results[1].url}`, value: results[1].description },
                        { name: `${results[2].title}\n${results[2].url}`, value: results[2].description },
                        { name: `${results[3].title}\n${results[3].url}`, value: results[3].description },
                        { name: `${results[4].title}\n${results[4].url}`, value: results[4].description },
                    ],
                    color: 0xEE9468,
                },
            });
